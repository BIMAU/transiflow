...
class BoundaryConditions:
    ...
    def moving_lid_west(self, atom, velocity):
        frc = self._constant_forcing_west(atom[:, :, :, :, 1, :, :, :], 1, 2 * velocity, -1)
        if self.dim == 3 and self.nz > 1:
            frc += self._constant_forcing_west(atom[:, :, :, :, 1, :, :, :], 2, 2 * velocity, -1)
    ...
    def moving_lid_south(self, atom, velocity):
        frc = self._constant_forcing_south(atom[:, :, :, :, 0, :, :, :], 0, 2 * velocity, -1)
    ...
    def moving_lid_bottom(self, atom, velocity):
        frc = self._constant_forcing_bottom(atom[:, :, :, :, 0, :, :, :], 0, 2 * velocity, -1) + \
...
# TODO: These methods are untested with nonzero heat flux and stretched grids
    ...
    def temperature_west(self, atom, temperature):
        '''T[i] + T[i-1] = 2 * Tb
        so T[i-1] = 2 * Tb - T[i]'''
        return self._constant_forcing_west(atom[:, :, :, :, self.dim+1, :, :, :], self.dim+1, 2 * temperature, -1)
    ...
    def temperature_south(self, atom, temperature):
        '''T[j] + T[j-1] = 2 * Tb
        so T[j-1] = 2 * Tb - T[j]'''
        return self._constant_forcing_south(atom[:, :, :, :, self.dim+1, :, :, :], self.dim+1, 2 * temperature, -1)
    ...
    def temperature_bottom(self, atom, temperature):
        '''T[k] + T[k-1] = 2 * Tb
        so T[k-1] = 2 * Tb - T[k]'''
        return self._constant_forcing_bottom(atom[:, :, :, :, self.dim+1, :, :, :], self.dim+1, 2 * temperature, -1)
    ...
    def _constant_forcing_west(self, atom, var, forcing_constant, atom_constant):
        frc = numpy.zeros((self.nx, self.ny, self.nz, self.dof))
        frc[0, :, :, :] = self._constant_forcing(atom[0, :, :, :, 0, :, :], self.ny, self.nz,
                                                 var, forcing_constant)
